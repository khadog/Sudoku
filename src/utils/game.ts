...
const inSelectBoxRange = (
  ...
  return (
    selected.colIndex >= startCol &&
    selected.colIndex <= endCol &&
    selected.rowIndex >= startRow &&
    selected.rowIndex <= endRow
  );
};
...
const isSameValue = (cell: CellType, selected: CellType): boolean => {
  return (
    selected.inputValue !== "" &&
    cell.inputValue !== "" &&
    selected.inputValue === cell.value
  );
};
...
const isGameFinished = (startingBoardMap: { [key: string]: any }): boolean => {
  return !Object.keys(startingBoardMap).some(
    (key) => startingBoardMap[key].value !== +startingBoardMap[key].inputValue
  );
};
